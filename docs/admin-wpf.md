...
WPF admin authentication is performed through the backend API. The visual login screen is available and includes a hide/show toggle for the password field. Bearer-token propagation to protected admin API calls is still a follow-up task. Development credentials are `admin` / `Admin@123456`. Identity hashes the password before database storage; plaintext passwords are never stored in the database. Replace development credentials and JWT configuration outside local development.
