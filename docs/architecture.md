...
Order numbers are generated by the Application layer from the Persian business year plus a yearly counter. For example, the first order in Persian year `1405` is `14051`, the second is `14052`, and the first order in `1406` is `14061`.
